...
// once the usb is initialised this timer is disabled
  ...
  static constexpr auto PARAM_FETCH_TIMEOUT = std::chrono::seconds(15);
  ...
  void ubx_rxm_in_frame(ubx_queue_frame_t * f)
  {
    switch (f->ubx_frame->msg_id) {
      ...
      case ubx::UBX_RXM_SFRBX:
        // Raw navigation subframe data — not processed, silently ignore
        RCLCPP_DEBUG(
          get_logger(), "ubx class: 0x%02x id: 0x%02x rxm sfrbx received, ignoring",
          f->ubx_frame->msg_class,
          f->ubx_frame->msg_id);
        break;
      default:
