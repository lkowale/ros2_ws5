...
def _make_spawn_node(context, *args, **kwargs):
    ...
    urdf_path = os.path.join(desc_dir, 'urdf', 'description.urdf')
...
def generate_launch_description():
    ...
    declare_robot_sdf_cmd = DeclareLaunchArgument(
        'robot_sdf', default_value='', description='Unused — URDF is read directly from description'
    )
    ...
    bridge = Node(
        ...
        namespace=namespace,
        parameters=[{
            'config_file': os.path.join(sim_dir, 'configs', 'gz_bridge.yaml'),
            'use_sim_time': False,
        }],
        output='screen',
    )
